...
class SpreadSheet
{
     ...
     * @return $this
     */
    public function addSheet($pIndex = null, $title = '')
    {
            ...
            foreach ($rowValue as $col => $val) {
                if ($this->isValidCoordinate($col)) {
                    ...
                    if (Coordinate::coordinateIsRange($col)) {
                        ...
                        $coordinate = Coordinate::extractAllCellReferencesInRange($col)[0];
                    }
                ...
                $this->activeSheet->setCellValue($coordinate, $val);
            }
    ...
    {
        return preg_match_all('/^[A-Z]{1,3}[1-9]{1,7}(:[A-Z]{1,3}[1-9]{1,7})?$/', $coordinate) === 1;
    }
     ...
     * @see setRowValue()
     */
    public function setRowValues(array $rowValues, $startRow = null)
    {
        if ($rowValues) {
            if (!$startRow) {
                $coor     = $this->activeSheet->getHighestRowAndColumn();
                $startRow = $coor['row'] == 1 && $coor['column'] == 'A' ? 1 : $coor['row'] + 1;
            }
            foreach ($rowValues as $rv) {
                $this->setRowValue($rv, $startRow);
                $startRow += 1;
            }
        }
    ...
    public function removeSheetByIndex($pIndex)
    {
        try {
            ...
            Logger::error("删除worksheet失败{$e->getMessage()}");
        }
